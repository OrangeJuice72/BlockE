...
export const saveLease = (tenant, unit, date) => {
  const leases = getLeases();
  leases.push({ id: Date.now(), tenant, unit, date });
...
export const generateICS = (tenant, unit, dateVal) => {
...
DESCRIPTION:Friendly reminder that the lease for ${tenant} (${unit}) is up!
BEGIN:VALARM
TRIGGER:-P1W
ACTION:DISPLAY
